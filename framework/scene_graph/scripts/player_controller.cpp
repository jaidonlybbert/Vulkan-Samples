...
void PlayerController::update(float delta_time)
{
	...
	if (delta_rotation != glm::vec3(0.0f, 0.0f, 0.0f) || delta_translation != glm::vec3(0.0f, 0.0f, 0.0f))
	{
		...
		transform.set_translation(transform.get_translation() + delta_translation * glm::conjugate(camera_twist));
		// transform.set_translation(transform.get_translation() + delta_translation);
